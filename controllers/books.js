    ...
    async.series([
        ...
        function(callback) {
            Video.find().find({'userId': req.session.passport.user}).distinct(internalGroupAttr, function (err, distinctGroups) {
                slingle.view(res, 'books/books', { gAttr: req.query.gAttr, distinctGroups: distinctGroups
    ...
    async.series([
        ...
        function(callback) {
            Video.where(internalGroupAttr, req.query.gName).find({'userId': req.session.passport.user}, function (err, videos) {
                slingle.view(res, 'books/bookDetails', { videos: videos }
...
function searchVideo(req, res) {
    ...
    Video.find({ }, function (err, videos) {
        slingle.view(res, 'books/searchResult', { videos: videos }
            , { messages: [{ type: "success", text: "Hey bro " + videos.length + " Videos found"}]
            });
    });
...
module.exports = function (app) {
    app.get('/books', isLoggedIn, index);
    app.get('/books/gGAttr',  isLoggedIn, getAllDistinctAttributes);
    app.get('/books/gVGAttr', isLoggedIn, getAllVideosPerGroupAttribute);

    app.post('/books/addVideo', isLoggedIn, addVideo);
    app.get('/books/searchVideo', isLoggedIn, searchVideo);

    app.post('/books/deleteVideo', isLoggedIn, deleteVideo);
...
/**
 * Created by dennismeyer on 14.04.14.
 */
